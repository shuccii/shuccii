...
from .theme import (ACCENT, CYAN, CYAN_D, DIM, FAINT, GOLD, HAIR, LINE, PALE,
                    RAMP, WHITE, atmosphere, defs, frame)
...
def _cells(x0: float, width: float, y_label: float, y_value: float,
    ...
    for i, (label, value) in enumerate(cells):
        ...
        if i:
            out.append(f'<line x1="{x-14:.1f}" y1="{y_label-11:.0f}" x2="{x-14:.1f}" '
                       f'y2="{y_value+5:.0f}" stroke="{DIM}" stroke-width="{HAIR}" opacity="0.55"/>')
        out.append(f'<text x="{x:.1f}" y="{y_label}" fill="{DIM}" font-size="8.5" '
                   f'letter-spacing="2.2">{label}</text>')
        out.append(f'<text x="{x:.1f}" y="{y_value}" fill="{PALE}" font-size="{value_size}" '
                   f'letter-spacing="0.4">{value}</text>')
    return "\n    ".join(out)


# --------------------------------------------------------------------------- header
def header(t: Telemetry) -> str:
    W, H, P = 900, 300, "h"
    CX, CY = 158, 150
    R_IN, R_MAX = 54, 116
    X0, XE = 312, 872
    ...
    for i, v in enumerate(weeks):
        ...
        spokes.append(
            ...
            f'stroke="{RAMP[lvl]}" stroke-width="2.4" stroke-linecap="round" '
        ...
        if t.week_starts:
            ...
            if m not in seen_months and i > 0:
                ...
                ticks.append(f'<line x1="{tx0:.1f}" y1="{ty0:.1f}" x2="{tx1:.1f}" y2="{ty1:.1f}" '
                             f'stroke="{DIM}" stroke-width="{HAIR}" opacity="0.85"/>')
                lx, ly = _pt(CX, CY, R_MAX + 19, deg)
                labels.append(f'<text x="{lx:.1f}" y="{ly+2.4:.1f}" fill="{DIM}" font-size="6.6" '
    ...
    peak_marker = (
        ...
        f'stroke="{GOLD}" stroke-width="{LINE}"/>'
        f'<circle cx="{px1:.1f}" cy="{py1:.1f}" r="1.7" fill="{GOLD}"/>'
        f'<text x="{plx:.1f}" y="{ply+2.4:.1f}" fill="{GOLD}" font-size="6.6" '
        f'letter-spacing="1" text-anchor="middle">PEAK {peak}</text>')

    scale = "\n      ".join(
        f'<line x1="{XE+6}" y1="{y}" x2="{XE + 6 + (14 if i % 4 == 0 else 7)}" y2="{y}" '
        f'stroke="{DIM}" stroke-width="{HAIR}" opacity="{0.85 if i % 4 == 0 else 0.45}"/>'
        for i, y in enumerate(range(74, 227, 9)))

    telemetry = _cells(X0, XE - X0, 200, 226, [
        ("COMMITS", f"{t.commits}"),
        ("PULL REQ", f"{t.prs}"),
        ("ISSUES", f"{t.issues}"),
        ("REPOS", f"{t.repos}"),
    ...
    live = (f'<text x="{XE}" y="272" text-anchor="end" fill="{CYAN}" font-size="9" letter-spacing="1.8">LIVE'
           ...
           f'<text x="{XE}" y="272" text-anchor="end" fill="{DIM}" font-size="9" letter-spacing="1.8">CACHED</text>'
...
<defs>{defs(P, W, H)}</defs>
<g clip-path="url(#{P}Clip)">
  <rect width="{W}" height="{H}" fill="url(#{P}Deep)"/>

  <!-- ============ CONTRIBUTION INSTRUMENT : one spoke per week ============ -->
  <g class="t">
    <circle cx="{CX}" cy="{CY}" r="{R_MAX+3}" fill="none" stroke="{FAINT}" stroke-width="{HAIR}" opacity="0.8"/>
    <circle cx="{CX}" cy="{CY}" r="{R_IN-4}" fill="none" stroke="{FAINT}" stroke-width="{HAIR}" opacity="0.8"/>
    <g>
    ...
    <g>
      ...
      <line x1="{CX}" y1="{CY-R_IN+6}" x2="{CX}" y2="{CY-R_MAX-4}" stroke="{CYAN}" stroke-width="{LINE}" opacity="0.9" filter="url(#{P}Glow)"/>
      <circle cx="{CX}" cy="{CY-R_MAX-8}" r="1.8" fill="{GOLD}" filter="url(#{P}Glow)"/>
    </g>

    <!-- orbital planes -->
    <g opacity="0.55">
      <g><animateTransform attributeName="transform" type="rotate" from="0 {CX} {CY}" to="360 {CX} {CY}" dur="38s" repeatCount="indefinite"/>
        <ellipse cx="{CX}" cy="{CY}" rx="{R_MAX+16}" ry="34" fill="none" stroke="{DIM}" stroke-width="{HAIR}"/></g>
      <g><animateTransform attributeName="transform" type="rotate" from="64 {CX} {CY}" to="424 {CX} {CY}" dur="52s" repeatCount="indefinite"/>
        <ellipse cx="{CX}" cy="{CY}" rx="{R_MAX+10}" ry="26" fill="none" stroke="{DIM}" stroke-width="{HAIR}" opacity="0.75"/></g>
    </g>

    <!-- core well -->
    <circle cx="{CX}" cy="{CY}" r="{R_IN-8}" fill="#03121b"/>
    <circle cx="{CX}" cy="{CY}" r="{R_IN-8}" fill="none" stroke="#17536b" stroke-width="2.6" filter="url(#{P}Bevel)"/>
    ...
    <g fill="none" stroke="{DIM}" stroke-width="{HAIR}" opacity="0.85">
      <path d="{_arc(CX, CY, R_IN-15, 14, 106)}"/>
      <path d="{_arc(CX, CY, R_IN-15, 134, 226)}"/>
      <path d="{_arc(CX, CY, R_IN-15, 254, 346)}"/>
    </g>

    <text x="{CX}" y="{CY+4}" text-anchor="middle" fill="{WHITE}" font-size="27" letter-spacing="0.5">{t.contributions}</text>
    <text x="{CX}" y="{CY+19}" text-anchor="middle" fill="{DIM}" font-size="6.8" letter-spacing="2.6">CONTRIBUTIONS</text>
    <text x="{CX}" y="{CY-24}" text-anchor="middle" fill="{DIM}" font-size="6.4" letter-spacing="2.4">LAST 12 MONTHS</text>

  </g>

  <!-- ============ READOUT ============ -->
  <g class="t">
    <text x="{X0}" y="60" fill="{GOLD}" font-size="10.5" letter-spacing="4.2">GITHUB.COM / {t.login.upper()}</text>
    <text x="{XE}" y="60" fill="{DIM}" font-size="10.5" letter-spacing="2.2" text-anchor="end">BRANCH {t.branch}</text>
    <line x1="{X0}" y1="70" x2="{XE}" y2="70" stroke="{DIM}" stroke-width="{HAIR}" opacity="0.5"/>

    <g font-size="38" letter-spacing="8.5">
      <text x="{X0-0.7}" y="122" fill="#ff4d4d" opacity="0.26">{t.name}</text>
      <text x="{X0+0.7}" y="122" fill="#3ee6ff" opacity="0.26">{t.name}</text>
      <text x="{X0}"     y="122" fill="{WHITE}">{t.name}</text>
    </g>

    <line x1="{X0}" y1="138" x2="{XE}" y2="138" stroke="{DIM}" stroke-width="{HAIR}" opacity="0.45"/>
    <line x1="{X0}" y1="138" x2="{X0+92}" y2="138" stroke="{GOLD}" stroke-width="{ACCENT}"/>

    <text x="{X0}" y="164" fill="#8ecfe6" font-size="11.5" letter-spacing="3">MATERIALS INFORMATICS &#183; MACHINE LEARNING &#183; FUSION</text>

    {telemetry}

    <line x1="{X0}" y1="248" x2="{XE}" y2="248" stroke="{DIM}" stroke-width="{HAIR}" opacity="0.45"/>
    <g font-size="9" letter-spacing="1.7" fill="{DIM}">
  ...
  <g>{scale}
    <line x1="{XE+4}" y1="74" x2="{XE+4}" y2="226" stroke="{DIM}" stroke-width="{HAIR}" opacity="0.6"/>
  </g>
{atmosphere(P, W, H)}
{frame(W, H)}
</g>
...
# --------------------------------------------------------------------------- command strip
def command_strip(t: Telemetry) -> str:
    ...
    for i, (cmd, out) in enumerate(rows):
        ...
        lines.append(
            ...
            f'<text x="96" y="22" fill="{PALE}" font-size="12.5" letter-spacing="0.4">{cmd}</text>'
            f'<text x="96" y="40" fill="{CYAN_D}" font-size="12.5" letter-spacing="0.4">{out}</text>'
            f'</g>')
...
<g clip-path="url(#{P}Clip)">
  <rect width="{W}" height="{H}" fill="url(#{P}Deep)"/>
  <line x1="78" y1="10" x2="78" y2="44" stroke="{DIM}" stroke-width="{HAIR}" opacity="0.6"/>
  <text class="t" x="24" y="22" fill="{GOLD}" font-size="10" letter-spacing="1.6">SHELL</text>
  <text class="t" x="24" y="40" fill="{DIM}" font-size="10" letter-spacing="1.6">STDOUT</text>
  <g class="t">
    {chr(10).join("    " + l for l in lines).strip()}
  </g>
  <rect x="{W-46}" y="21" width="6" height="12" fill="{GOLD}">
    <animate attributeName="opacity" values="1;1;0.1;0.1" keyTimes="0;0.5;0.55;1" dur="1.6s" repeatCount="indefinite"/>
  </rect>
  <circle cx="{W-24}" cy="27" r="3" fill="{CYAN}" filter="url(#{P}Glow)">
    <animate attributeName="opacity" values="0.95;0.3;0.95" dur="2.6s" repeatCount="indefinite"/>
  </circle>
{atmosphere(P, W, H, sweep_dur="7s")}
  <rect x="0.5" y="0.5" width="{W-1}" height="{H-1}" rx="3" fill="none" stroke="{DIM}" stroke-width="0.8" opacity="0.6"/>
</g>
</svg>
'''
...
<defs>
  <linearGradient id="{P}Rail" x1="0" y1="0" x2="1" y2="0">
    ...
    <stop offset="16%" stop-color="{CYAN}" stop-opacity="0.55"/>
    <stop offset="84%" stop-color="{CYAN}" stop-opacity="0.55"/>
    <stop offset="100%" stop-color="{CYAN}" stop-opacity="0"/>
  </linearGradient>
  <linearGradient id="{P}Pulse" x1="0" y1="0" x2="1" y2="0">
    <stop offset="0%" stop-color="{CYAN}" stop-opacity="0"/>
    <stop offset="50%" stop-color="#eafcff" stop-opacity="1"/>
    <stop offset="100%" stop-color="{CYAN}" stop-opacity="0"/>
  </linearGradient>
  ...
  <line x1="150" y1="11" x2="410" y2="11" stroke="{DIM}" stroke-width="4" stroke-dasharray="0.7 6" opacity="0.75"/>
  <line x1="490" y1="11" x2="750" y2="11" stroke="{DIM}" stroke-width="4" stroke-dasharray="0.7 6" opacity="0.75"/>
  <path d="M432 11 L442 5 L458 5 L468 11 L458 17 L442 17 Z" fill="none" stroke="{GOLD}" stroke-width="{HAIR}"/>
  <circle cx="450" cy="11" r="1.6" fill="{CYAN}" filter="url(#{P}Glow)">
...
def footer(t: Telemetry) -> str:
    ...
    for i, v in enumerate(weeks):
        ...
        bars.append(f'<line x1="{x:.1f}" y1="{base}" x2="{x:.1f}" y2="{y:.1f}" '
                    f'stroke="{RAMP[_level(v, peak)]}" stroke-width="1.6" opacity="0.75"/>')

    totals = _cells(560, 292, 52, 74, [
        ("REPOS", f"{t.repos}"),
        ("STARS", f"{t.stars}"),
...
<g clip-path="url(#{P}Clip)">
  ...
  <g class="t">
    <text x="{x0}" y="24" fill="{DIM}" font-size="8.5" letter-spacing="2.4">LAST 26 WEEKS</text>
    <line x1="{x0}" y1="{base}" x2="{x1}" y2="{base}" stroke="{DIM}" stroke-width="{HAIR}" opacity="0.7"/>
    <g>{chr(10).join("      " + b for b in bars).strip()}</g>
    <polyline points="{' '.join(pts)}" fill="none" stroke="{CYAN}" stroke-width="{LINE}" opacity="0.9"/>
    <line x1="{x0}" y1="{top-6}" x2="{x0}" y2="{base}" stroke="{DIM}" stroke-width="{HAIR}" opacity="0.5"/>
    <text x="{x0-6}" y="{top-1}" fill="{DIM}" font-size="7" text-anchor="end">{peak}</text>
    <text x="{x0-6}" y="{base+3}" fill="{DIM}" font-size="7" text-anchor="end">0</text>
    <line x1="524" y1="24" x2="524" y2="94" stroke="{DIM}" stroke-width="{HAIR}" opacity="0.55"/>
    {totals}
    <line x1="{x0}" y1="98" x2="{W-66}" y2="98" stroke="{DIM}" stroke-width="{HAIR}" opacity="0.45"/>
    <text x="{x0}" y="115" fill="{DIM}" font-size="9" letter-spacing="1.7">{t.login}/{t.login} &#183; regenerated daily by github actions</text>
    <text x="{W-66}" y="115" fill="{DIM}" font-size="9" letter-spacing="1.7" text-anchor="end">SYNC {t.synced}Z</text>
  </g>
{atmosphere(P, W, H, sweep_dur="8s")}
{frame(W, H, inset=10, arm=36)}
</g>
</svg>
'''
